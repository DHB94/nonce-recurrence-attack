// =============================================================================
// Arbitrum Flash Sandwich Bot - FIXED VERSION
// Monitors mempool for V2 swaps, simulates sandwich profitability,
// executes atomic front-run + back-run via Aave V3 flash loan.
// =============================================================================
//
// FIXES APPLIED:
// 1. Fixed Solidity contract: swapA -> swapB in back-run Balancer V2 logic
// 2. Fixed ICurveCryptoPool interface: uint264 -> uint256
// 3. Added noReentrant modifier to initiateSandwich
// 4. Added noReentrant modifier to emergencyWithdraw
// 5. Fixed PAIR_ABI: uint122 -> uint112
// 6. Removed duplicate PROFIT_JSON/PROFIT_CSV constants
// 7. Added chain validation (Arbitrum mainnet = 42161)
// 8. Fixed gas price settings for Arbitrum
// 9. Added gas cost estimation to profitability simulation
// 10. Added minimum reserve checks
//
// =============================================================================
...
if (RPC_LIST.length === 0) {
  console.error("❌ Missing RPC_LIST or WRITE_RPC in .env");
  process.exit(1);
}
...
// Sandwich parameters - FIXED: More realistic gas prices for Arbitrum
const SLIPPAGE_BPS = BigInt(process.env.SLIPPAGE_BPS || "50"); // 0.5%
const MAX_BORROW_BPS = BigInt(process.env.MAX_BORROW_BPS || "4200"); // 42%
const MIN_PROFIT_BPS = BigInt(process.env.MIN_PROFIT_BPS || "10"); // 0.1%
const GAS_LIMIT = parseInt(process.env.GAS_LIMIT || "1200000", 10);
const PRIORITY_FEE_FLOOR_GWEI = process.env.PRIORITY_FEE_FLOOR_GWEI || "0.1"; // FIXED: Higher floor
...
// FIXED: Added gas cost estimation
const ESTIMATED_GAS_COST_WEI = 3_000_000n; // ~3M gas for sandwich tx
const ETH_PRICE_USD = 2000; // Conservative ETH price estimate
...
// =============================================================================
// FIXED: Solidity Contract with All Bug Fixes
// =============================================================================
...
// FIXED: Changed to inherit from ReentrancyGuard instead of custom noReentrant
contract FlashSandwichMultiVenue is FlashLoanReceiverBase, ReentrancyGuard {
...
// =============================================================================
// Deploy - FIXED: Higher gas prices for Arbitrum
// =============================================================================
...
async function deploy(provider, wallet, force) {
  ...
  // FIXED: More realistic gas prices for Arbitrum
  const gasSettings = [
    { gasLimit: 5_000_000, gasPrice: ethers.parseUnits("0.5", "gwei") },
    { gasLimit: 6_000_000, gasPrice: ethers.parseUnits("1", "gwei") },
    { gasLimit: 7_000_000, gasPrice: ethers.parseUnits("2", "gwei") },
    { gasLimit: 8_000_000, gasPrice: ethers.parseUnits("5", "gwei") },
    { gasLimit: 10_000_000, gasPrice: ethers.parseUnits("10", "gwei") },
  ];
...
const ROUTERS = [
  ...
  // FIXED: Added more Arbitrum routers
  { name: "Camelot", address: "0xc873fEcbd354f5A5186b7494541e5b3596b448980" }
];
...
// =============================================================================
// FIXED: Sandwich Simulation with Gas Cost Estimation
// =============================================================================
...
function simulateSandwich(borrowAmount, victimAmount, reserveIn, reserveOut, premiumBps) {
  ...
  // FIXED: Estimate gas cost in token units
  // Assuming ETH price = $2000, token price = $1
  const gasCostWei = ESTIMATED_GAS_COST_WEI * ethers.parseUnits("1", "gwei"); // 3M gas * 1 gwei
  const ETH_PRICE_USD = 2000n * 10n ** 18n;
  const TOKEN_PRICE_USD = 1n * 10n ** 18n; // Assume $1 token
  ...
  // FIXED: Include gas cost in net profit calculation
  const netProfit = backOut - borrowAmount - flashLoanFee - gasCostToken;
  if (netProfit <= 0n) return null;

  const minFrontOut = (frontOut * (10_000n - SLIPPAGE_BPS)) / 10_000n;
  const minBackOut = (() => { 
    const slipped = (backOut * (10_000n - SLIPPAGE_BPS)) / 10_000n; 
    const totalOwed = borrowAmount + flashLoanFee;
    return slipped > totalOwed ? slipped : totalOwed; 
  })();

  return {
    borrowAmount, flashLoanFee, gasCostToken, frontOut, victimOut, backOut, netProfit,
...
// =============================================================================
// FIXED: Sandwich Evaluation with Minimum Reserve Check
// =============================================================================
...
async function evaluateSandwich(tx, parsed, routerConfig, provider, premiumBps) {
  ...
  // FIXED: Add minimum reserve check
  const tokenInMeta = await getTokenMeta(tokenIn, provider);
  const tokenOutMeta = await getTokenMeta(tokenOut, provider);
  const minReserveIn = ethers.parseUnits(String(MIN_RESERVE_USD), tokenInMeta.decimals);
  const minReserveOut = ethers.parseUnits(String(MIN_RESERVE_USD), tokenOutMeta.decimals);
  
  if (reserveIn < minReserveIn || reserveOut < minReserveOut) {
    console.log(`⚠️ Skipping: reserves too small (${formatUnits(reserveIn, tokenInMeta.decimals)} ${tokenInMeta.symbol}, ${formatUnits(reserveOut, tokenOutMeta.decimals)} ${tokenOutMeta.symbol})`);
    return null;
  }
  ...
  // FIXED: Also check victim size meets minimum
  const victimInFormatted = formatUnits(victimIn, tokenInMeta.decimals);
  if (parseFloat(victimInFormatted) < MIN_VICTIM_USD) {
    console.log(`⚠️ Skipping: victim trade too small (${victimInFormatted} ${tokenInMeta.symbol})`);
    return null;
  }
  ...
  return {
    ...
    gasCostToken: best.gasCostToken,
    profitBps,
    victimTx: tx
  };
}
...
// =============================================================================
// Main - FIXED: Added Chain Validation
// =============================================================================
...
async function main() {
  ...
  if (network.chainId !== BigInt(ARBITRUM_CHAIN_ID)) {
    console.error(`❌ Wrong chain! Expected Arbitrum (${ARBITRUM_CHAIN_ID}), got ${network.chainId}`);
    process.exit(1);
  }
  ...
  try {
    ...
    console.warn("⚠️ Could not read premium, defaulting to 9 bps");
    premiumBps = 9n; // Aave V3 default
  }
  ...
  async function handlePendingTx(txHash) {
    try {
      ...
      const gasCost = formatUnits(opportunity.gasCostToken, opportunity.assetMeta.decimals);
      console.log(
        `🥪 [${opportunity.assetMeta.symbol}] victim=${victim} borrow=${borrow} ` +
        `profit=${profit} gasCost=${gasCost} edge=${opportunity.profitBps} bps via ${routerConfig.name}`
      );
  ...
  if (wsProvider) {
    ...
    console.log(`💰 Gas settings: floor=${PRIORITY_FEE_FLOOR_GWEI} gwei, multiplier=${PRIORITY_FEE_MULTIPLIER}x`);
    ...
    wsProvider.websocket?.on?.("close", async () => {
      ...
      await sleep(5000);
      try {
        // FIXED: Clean up old listeners
        if (wsProvider) {
          wsProvider.off("pending", handlePendingTx);
        }
        wsProvider = new ethers.WebSocketProvider(WSS_URL, undefined, { timeout: 30_000 });
        wsProvider.on("pending", handlePendingTx);
